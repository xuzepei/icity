...
{
    ADWOSDK_FSAD_SHOW_FORM_APPFUN_WITH_BRAND,   // App Fun加全屏插屏（App Fun优先）
    ADWOSDK_FSAD_SHOW_FORM_LAUNCHING,           // 应用启动后立即展示全屏广告(开机全屏)
    ADWOSDK_FSAD_SHOW_FORM_GROUND_SWITCH,       // 后台切换到前台后立即显示全屏广告(前后台切换全屏)
    ADWOSDK_FSAD_SHOW_FORM_APPFUN,              // AppFun(插屏居中)
    ADWOSDK_FSAD_SHOW_FORM_BRAND                // 品牌插页式全屏(全屏插屏)
};
